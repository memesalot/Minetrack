      ...
      serverRegistration.dnsResolver.resolve((host, port, remainingTimeout) => {
        ...
        try {
          server.ping(remainingTimeout, version, (err, res) => {
            if (err) {
              // Some servers return malformed JSON and mcping-js surfaces it as a parse error.
              // Normalize those into a simple error to avoid noisy stack traces.
...
function capPlayerCount (host, playerCount) {
  ...
  if (!Number.isFinite(playerCount) || playerCount < 0) {
    logger.log('warn', '%s returned an invalid player count of %s, setting to 0.', host, playerCount)
    return 0
  }
