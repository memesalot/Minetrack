...
const MAX_FAVICON_BYTES = 200 * 1024 // 200KB safety cap

function decodeFavicon (favicon) {
  ...
  try {
    const buf = Buffer.from(favicon.substring(prefix.length), 'base64')
    if (!buf.length || buf.length > MAX_FAVICON_BYTES) {
      return
    }
    return buf
  } catch (e) {
...
class ServerRegistration {
  ...
  updateFavicon (favicon) {
    ...
    if (favicon && favicon !== this.lastFavicon) {
      const buf = decodeFavicon(favicon)
      if (!buf) {
        return false
      }

      this.lastFavicon = favicon
      this.lastFaviconBuffer = buf
