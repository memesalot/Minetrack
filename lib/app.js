...
class App {
  ...
  checkMessageRateLimit (client) {
    const limits = config.wsRateLimits || {}
    const maxMessages = limits.maxMessagesPerWindow || 10
    const windowMs = limits.windowMs || 60000

    const now = Date.now()
    if (now - client._messageResetTime > windowMs) {
      client._messageCount = 0
      client._messageResetTime = now
    }

    client._messageCount++
    if (client._messageCount > maxMessages) {
